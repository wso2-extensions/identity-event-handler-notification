...
public class TokenManagerTest {
    ...
    public void testAuthenticationWithClientCredential() throws Exception {
        ...
        try (MockedStatic<NotificationSenderUtils> mockedUtils = mockStatic(NotificationSenderUtils.class)) {
            mockedUtils.when(() -> NotificationSenderUtils.buildAuthenticationHeader(
                    eq(Authentication.Type.CLIENT_CREDENTIAL), any())).thenReturn(mockHeader);
    ...
    public void testAuthenticationWithBasicType() throws Exception {
        ...
        try (MockedStatic<NotificationSenderUtils> mockedUtils = mockStatic(NotificationSenderUtils.class)) {
            mockedUtils.when(() -> NotificationSenderUtils.buildAuthenticationHeader(
                    eq(Authentication.Type.BASIC), any())).thenReturn(mockHeader);
    ...
    public void testAuthenticationWithBearerType() throws Exception {
        ...
        try (MockedStatic<NotificationSenderUtils> mockedUtils = mockStatic(NotificationSenderUtils.class)) {
            mockedUtils.when(() -> NotificationSenderUtils.buildAuthenticationHeader(
                    eq(Authentication.Type.BEARER), any())).thenReturn(mockHeader);
    ...
    public void testAuthenticationInternalProperties() throws Exception {
        ...
        try (MockedStatic<NotificationSenderUtils> mockedUtils = mockStatic(NotificationSenderUtils.class)) {
            ...
            Assert.assertEquals(auth.getProperty("AccessToken"), "test-access-token");
            Assert.assertEquals(auth.getProperty("RefreshToken"), "test-refresh-token");
        }
    }
